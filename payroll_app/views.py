...
import datetime
import calendar
from .models import year_choices, current_year

# from django.contrib.auth.hashers import make_password
# from django.contrib.auth.hashers import check_password
# if employer and check_password(password, employer.password):
...
from payroll_app.seralizers import PositionSerializer

from payroll_app.seralizers import UserLoginSerializer

from payroll_app.seralizers import EmployerLoginSerializer
# from payroll_app.seralizers import VerifyUsersByIdSerializer

from payroll_app.models import PayrollManagement
from payroll_app.seralizers import PayrollManagementSerializer

from payroll_app.seralizers import AnnualSalarySerializer

from payroll_app.seralizers import PayrollCalculationSerializer

from payroll_app.models import LeaveManage
from payroll_app.seralizers import LeaveManageSerializer
from payroll_app.seralizers import LeaveApplySerializer

from payroll_app.seralizers import UpdateStatusSerializer
...
def verify_users_by_id(request, user_id):
    ...
    if user.verified == False:    
        serializer = AnnualSalarySerializer(data=request.data)
        if serializer.is_valid():
            user.verified = True
            user.annual_salary = serializer.validated_data['annual_salary']
            user.save()
            serializer = UserSerializer(user)
        return Response(serializer.data, status=status.HTTP_200_OK)
    else:
        ...
        if serializer.is_valid(): 
            ...
            if status_value == 'approved':
                if user.leaves > 0:
                    ...
                    user.leaves -= 1
                    user.save()
                    leaves_data.status = 'approved'
                    leaves_data.save()
                    return Response({"detail" : "User does not have enough leaves but if this leave is of los-of-pay then it is approved"}, status=status.HTTP_400_BAD_REQUEST)
...
@swagger_auto_schema(methods=['get'])
@api_view(['GET'])  
def user_payrolls(request, user_id):
    try:
        user = User.objects.get(pk=user_id)
    except User.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)
    ...
    if request.method == 'GET':
        if user.verified:    
            all_data = PayrollManagement.objects.filter(user=user)
            serializer = PayrollManagementSerializer(all_data, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
